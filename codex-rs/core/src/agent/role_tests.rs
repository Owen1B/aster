...
async fn apply_role_preserves_unspecified_keys() {
    ...
    .await;
    config.codex_linux_sandbox_exe = Some(PathBuf::from("/tmp/aster-linux-sandbox"));
    config.main_execve_wrapper_exe = Some(PathBuf::from("/tmp/aster-execve-wrapper"));
    let role_path = write_role_config(
        &home,
        "effort-only.toml",
    ...
    assert_eq!(
        ...
        Some(PathBuf::from("/tmp/aster-linux-sandbox"))
    );
    assert_eq!(
        config.main_execve_wrapper_exe,
        Some(PathBuf::from("/tmp/aster-execve-wrapper"))
    );
}
